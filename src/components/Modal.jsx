...
const Modal = ({toggleModal}) => {
  ...
  return (
    <div onClick={toggleModal}  className=" w-[100%] max-w-[inherit] h-full bg-[#000000b0] fixed z-[999] flex justify-center items-center">
     <div onClick={wand}  className="w-[95%] md:w-[950px] h-[95vh] md:h-[610px]  bg-white flex flex-col md:flex-row rounded-[25px] md:overflow-hidden">
          <section className="  md:w-[50%] px-[30px] pb-[20px]  flex flex-col items-center">
              ...
              <div className="w-full h-[70px] mt-2 text-[14px] flex items-center">
                <span onClick={toggleInput} className={`  w-[195px] mr-2  py-3 pl-2 flex items-center rounded-full cursor-pointer   ${isBuyer? "bg-[#FFF2EF]": "bg-[#f7f7f7]"} `}> <span className=" mr-1"> {isBuyer? <CheckedSvg/>: <EmptyCheckSvg/>} </span>  I’m a Buyer</span>
                <span onClick={toggleInput} className={` w-[195px] ml-[auto] flex items-center  py-3 pl-2 rounded-full cursor-pointer  ${isBuyer? "bg-[#F7F7F7]": "bg-[#FFF2EF]"}`}> <span className=" mr-1"> {isBuyer? <EmptyCheckSvg/>: <CheckedSvg/>} </span>  I’m a Seller</span>
              </div>
 
              <p className=" px-5 text-[15px] mt-3 text-center font-extralight ">
            {isBuyer? ("Ginger allows you to buy a variety of quality products from a large network of vetted beauty and fashion merchants and manufacturers.")
          ...
          <div className=" w-full h-fit mt-6 ">
                <form action="" className=" h-[370px]  flex flex-col ">
                  <label className="flex text-[14px]  pt-[5px] pl-[10px] text-[#6E6E6E] mb-[15px] border-b-[1.5px] rounded-t-[10px] flex-col bg-[#f7f7f7]">
                    Full Name:
                    <input type="text" className="h-[25px] text-[16px] text-black pb-[5px] pt-[5px] bg-[#f7f7f7] mb-[5px]  outline-none m-[#f2f2f2] "/>
                  </label>
                  ...
                  <span className=" m-auto w-fit inline-block md:mb-[40px]" onClick={(e)=> e.preventDefault()}><PillButton text={'Join The Waitlist'}/></span>
                </form>
          </div>
          </section>
